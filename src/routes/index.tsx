...
import { PageBlockRenderer, type EditorContent } from '#/components/PageBlockRenderer'

const getHomeContent = createServerFn().handler(async () => {
...
function Home() {
	const content = Route.useLoaderData().content as EditorContent | null
		...
		<main className="mx-auto max-w-3xl px-6 py-32 md:px-12">
			<div className="mb-16 flex justify-center">
				<img
					src="/rv-logo.png"
					alt="Red Vandet"
					className="w-[calc(100vw-3rem)] max-w-[500px]"
				/>
			</div>
			...
			{content ? (
				<PageBlockRenderer content={content} />
			) : (
				<StaticContent />
			)}
		</main>
